 ...
 * Modified Code (C) Copyright Renesas Electronics Corporation 2024-2025
 *　
...
* File Name    : define.h
* Version      : 1.0.0
* Description  : DRP-AI TVM[*1] Application for Face Mosaic
 ...
 ******************************************/
/* Model Binary */
const static std::string model_dir = "face_mosaic_yolov3";
...
#define DRPAI_MEM_OFFSET (0x700000)
...
const static uint32_t INF_OUT_SIZE = (NUM_CLASS + 5) * NUM_BB * num_grids[0] * num_grids[0] + (NUM_CLASS + 5) * NUM_BB * num_grids[1] * num_grids[1] + (NUM_CLASS + 5) * NUM_BB * num_grids[2] * num_grids[2];
...
#define DRPAI_IN_HEIGHT (IMAGE_WIDTH) // to set the image aspect ratio to 1:1.
#define BOX_IN_WIDTH (IMAGE_WIDTH)
#define BOX_IN_HEIGHT (IMAGE_HEIGHT)
...
#define D_TIME_STR_X (645)
#define D_TIME_STR_Y (370)
