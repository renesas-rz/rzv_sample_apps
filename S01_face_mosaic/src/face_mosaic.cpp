 ...
 * Modified Code (C) Copyright Renesas Electronics Corporation 2023-2025
 *　
...
void draw_bounding_box(void)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        if ((x_mosaic + y_mosaic) < 300) // Optional threshold to change mosaic intensity
        {
            mosaic_rate = 10; // Reduced to 1/10
        }
        else
        {
            mosaic_rate = 20; // Reduced to 1/20
        }
        ...
        if (box_mode == 1)
        {
            ...
            rectangle(img_frame, textleft, textright, Scalar(59, 94, 53), -1); // Scalar is dark green color
...
int8_t R_Main_Process()
{
    ...
    while (1)
    {
        ...
        putText(output_image, str, Point((DISP_OUTPUT_WIDTH - draw_size.width - RIGHT_ALIGN_OFFSET), (D_TIME_STR_Y + draw_size.height)), FONT_HERSHEY_SIMPLEX,
                CHAR_SCALE_LARGE, Scalar(0, 0, 0), 1.5 * HC_CHAR_THICKNESS);
        putText(output_image, str, Point((DISP_OUTPUT_WIDTH - draw_size.width - RIGHT_ALIGN_OFFSET), (D_TIME_STR_Y + draw_size.height)), FONT_HERSHEY_SIMPLEX,
                CHAR_SCALE_LARGE, Scalar(0, 255, 0), HC_CHAR_THICKNESS);
...
#ifdef V2H
    unsigned long OCA_list[DRP_FUNC_NUM]; // The number of elements is the number of libraries in OpenCV Accelerator.
...
/*Load model_dir for DRP-AI inference */
    ...
    ret = buffer_alloc_dmabuf(drpai_buf, DRPAI_IN_WIDTH * DRPAI_IN_HEIGHT * BGR_CHANNEL);
    if (-1 == ret)
    {
    ...
    case 1:
    {
        ...
        gstreamer_pipeline = "v4l2src device=" + media_port + " ! video/x-raw, width="+std::to_string(IMAGE_WIDTH) +", height="+std::to_string(IMAGE_HEIGHT) +", framerate=30/1 ! videoconvert ! appsink";

        /* Initialize waylad */
        ret = wayland.init(drpai_buf->idx, DISP_OUTPUT_WIDTH, DISP_OUTPUT_HEIGHT, BGRA_CHANNEL);
        if (0 != ret)
        {
